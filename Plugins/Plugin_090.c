...
boolean Plugin_090(byte function, char *string) {
  boolean success=false;
      ...
      if (RawSignal.Number!=NodoSlave_PULSECOUNT )           // Per byte twee posities + startbit.
         return false;
...
//#define NODO_TYPE_PLUGIN_COMMAND                         5
      if ((DataBlock.Command == 4) && (DataBlock.Type == 1)){  // event->command = DataBlock.Command = 4 = EVENT_VARIABLE
         ...
         Serial.print( pbuffer );
         //==================================================================================
         if ((DataBlock.Par1 < 11)  || (DataBlock.Par1 > 16)) {  // Unsupported (for now) variable, just show the contents
